...
# Upload section with HCL logo
st.sidebar.image("https://upload.wikimedia.org/wikipedia/commons/thumb/3/33/HCL_Technologies_Logo.svg/2560px-HCL_Technologies_Logo.svg.png", use_column_width=True)
...
if df is not None:
    ...
    try:
        ...
        if query:
            ...
            if any(x in df.columns for x in ['deployment status', 'training status']):
                ...
                if 'deployment status' in df.columns:
                    if "on bench" in query or "bench" in query:
                ...
                if 'training status' in df.columns:
                    if "completed seer" in query:
                        ...
                        response = f"🎓 {len(result)} completed SEER training."
                        matched = True
                    ...
                    elif "in training" in query:
                        result = df[df['training status'].str.lower() == "training in progress"]
                        response = f"🧠 {len(result)} currently in training."
                        matched = True
                ...
                if 'department' in df.columns:
                    if "pie chart of department" in query:
                ...
                if 'deployment status' in df.columns and ("deployment status chart" in query):
            ...
            if talent_name:
                if "department" in query and 'talent name' in df.columns and 'department' in df.columns:
                    ...
                    if not row.empty:
                        ...
                        response = f"❌ Talent '{talent_name.title()}' not found."
                    matched = True
                    ...
                    if not row.empty:
                        ...
                        response = f"❌ Email for '{talent_name.title()}' not found."
                    matched = True

            if response:
                st.text_area("🤖 Bot Response", value=response, height=150)
            elif not matched:
                st.text_area("🤖 Bot Response", value="🤖 Sorry, I couldn’t understand that question.", height=150)
        ...
        st.warning(f"❌ Error rendering dashboard: {e}")
